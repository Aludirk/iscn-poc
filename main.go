...
import (
	...
	iscnentity "github.com/likecoin/iscn-ipld/plugin/block/entity"
...
func testEntity(
	...
	b1, err := iscnentity.NewEntityBlock(1, data)
	if err != nil {
	...
	b2, err := iscnentity.NewEntityBlock(1, data)
	if err != nil {
	...
	b3, err := iscnentity.NewEntityBlock(1, data)
	if err != nil {
	...
	if !ok {
		log.Panicf("Cannot convert to \"IscnObject\" for entity 1")
	}
	...
	if !ok {
		log.Panicf("Cannot convert to \"IscnObject\" for entity 2")
	}
	...
	d3, err := iscn.Decode(ret3.RawData(), b3.Cid())
	if err != nil {
		log.Panicf("Cannot decode IPLD raw data: %s", err)
	}

	obj3, ok := d3.(iscn.IscnObject)
	if !ok {
		log.Panicf("Cannot convert to \"IscnObject\" for entity 3")
	}
	...
	if val, err := obj1.GetString("id"); ok {
		log.Printf("  ID: %q", val)
	} else {
		log.Panicf("%s", err)
	}
	...
	if val, err := obj2.GetString("id"); ok {
		log.Printf("  ID: %q", val)
	} else {
		log.Panicf("%s", err)
	}
	...
	if val, err := obj3.GetString("id"); ok {
		log.Printf("  ID: %q", val)
	} else {
		log.Panicf("%s", err)
	}
